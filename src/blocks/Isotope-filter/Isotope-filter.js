...
function isotopInit(arg) {
	...
	$('#filters').on('click', 'button', function() {
		...
		$elm.isotope({ filter: filterValue });
	});
	// bind sort button click
	$('#sorts').on('click', 'button', function() {
		var sortByValue = jQuery(this).attr('data-sort-by');
		$elm.isotope({ sortBy: sortByValue });
	});
...
function isotopeInit3() {
	// init Isotope
	var $grid2 = $('.isotop-g2').isotope({
		itemSelector: '.isotop-g-item2',
		layoutMode: 'fitRowsHeight',
		fitRowsHeight: {
			equalheight: true,
			gutter: {
				width: 30,
	...
	// bind filter on select change
	$('.isotop-g-select2').on('change', function() {
		// get filter value from option value
		var filterValue = this.value;
...
// #new options for LayoutMode
(function() {
	var FitRowsHeight = window.Isotope.LayoutMode.create('fitRowsHeight');
	FitRowsHeight.prototype._resetLayout = function() {
		this.x = 0;
		this.y = 0;
		this.maxY = 0;
		this.row = 0;
	...
	FitRowsHeight.prototype._getItemLayoutPosition = function(item) {
		// Initialize vars
		...
		// if this element cannot fit in the current row
		var containerWidth = Math.ceil(this.isotope.size.innerWidth + (this.gutter.width || 0));
		if (this.x !== 0 && itemWidth + this.x > containerWidth) {
			this.x = 0;
			this.y = this.maxY;
		}
	...
	FitRowsHeight.prototype._equalHeight = function() {
		// console.log('---')
		for (var i = 0; i < this.items.length; i++) {
			var row = this.items[i].row,
				data = this.rows[row];
			if (data) {
				var height = data.end - data.start;
				height -= this.items[i].size.borderTopWidth - this.items[i].size.borderBottomWidth;
				height -= this.items[i].size.marginTop - this.items[i].size.marginBottom;
				height -= this.gutter.height || 0;
				if (this.items[i].size.isBorderBox == false) {
					height -= this.items[i].size.paddingTop - this.items[i].size.paddingBottom;
				}
				this.items[i].size.height = height;
				this.items[i].css({
	...
	FitRowsHeight.prototype._getContainerSize = function() {
		this._equalHeight()
		return { height: this.maxY };
	};
